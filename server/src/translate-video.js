...
export async function explainWord(word, context, { model = 'sonnet' } = {}) {
  // «Выражение», не «слово»: сюда приходят и словосочетания («plow through»,
  // «get away with») — их надо переводить как целое, а не пословно.
  const reply = await runClaude(
    `Выражение: ${word}\nФраза из видео: ${context}\n\nВерни JSON: {"tr": "перевод именно в этом значении, 1–4 слова", "note": "одно короткое пояснение оттенка или типичного употребления"}`,
    {
      system: 'Ты помогаешь учить английский. Переводишь слово или словосочетание в том значении, ' +
        'в котором оно употреблено во фразе, а не первым словарным. Отвечаешь только JSON-объектом.',
      model,
      timeoutMs: 90_000,
    })
  const obj = parseJsonReply(reply)
